...
@section('content')
<div class="bg-gradient-to-b from-[#dff3ea] to-[#faf8f6] py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
            ...
            <div class="space-y-6">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    @foreach ($product->sizes as $size)
                        <span class="px-2 py-1 border rounded">{{ $size->size }}</span>
                    @endforeach
                </div>
                ...
                {{-- Add to Cart --}}
                <button class="bg-[#00cccc] hover:bg-[#00aaaa] text-white px-6 py-3 rounded-lg font-semibold w-full mx-auto block">
                    Add to Cart
                </button>
        ...
        <div x-data="reviewsManager(@js($product->slug))" x-init="init()" class="space-y-6">
            <div class="flex justify-between items-center">
                ...
                @if($userCanReview)
                    <button @click="writeReviewOpen = true" class="bg-[#00cccc] hover:bg-[#00aaaa] text-white px-4 py-2 rounded-lg">
                        Write a Review
                    </button>
                @endif
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                ...
                <div class="w-full lg:w-3/4 space-y-4">
                    ...
                    <button x-show="moreReviews.length > 0" @click="loadMore()" class="bg-[#00cccc] hover:bg-[#00aaaa] text-white px-4 py-2 rounded-lg">
                        Load More Reviews
                    </button>
                </div>
            </div>
            ...
            <!-- Write Review Modal -->
            @if($userCanReview)
            <div x-show="writeReviewOpen" x-cloak class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
                <div class="bg-white p-6 rounded-lg w-full max-w-lg">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-xl font-bold">Write a Review</h3>
                        <button @click="writeReviewOpen = false" class="text-gray-600 text-2xl">&times;</button>
                    </div>

                    <form @submit.prevent="submitReview">
                        <div class="mb-4">
                            <label class="block text-gray-700">Rating</label>
                            <select x-model="newReview.rv_rate" class="w-full border rounded p-2">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea x-model="newReview.rv_comment" class="w-full border rounded p-2" rows="4"></textarea>
                        </div>
                        <div class="flex justify-end space-x-2">
                            <button type="button" @click="writeReviewOpen = false" class="px-4 py-2 rounded bg-gray-300 hover:bg-gray-400">Cancel</button>
                            <button type="submit" class="px-4 py-2 rounded bg-[#00cccc] hover:bg-[#00aaaa] text-white">Submit</button>
                        </div>
                    </form>
                </div>
            </div>
...
function reviewsManager(productSlug) {
    return {
        ...
        newReview: { rv_rate: 5, rv_comment: '', customer_name: 'You' },
        ...
        loadMore() {
            this.displayedReviews.push(...this.moreReviews.splice(0, 5));
        },
        ...
        submitReview() {
            fetch(`/api/products/${productSlug}/reviews`, {
                ...
                body: JSON.stringify({
                    rv_rate: this.newReview.rv_rate,
                    rv_comment: this.newReview.rv_comment
                })
            })
            .then(res => {
                if (!res.ok) throw new Error('You must be logged in to submit a review.');
                return res.json();
            })
            .then(data => {
                const newR = {
                    id: data.id,
                    rv_rate: data.rv_rate,
                    rv_comment: data.rv_comment,
                    customer_name: data.customer_name || 'You'
                };
                this.reviews.unshift(newR);
                this.displayedReviews.unshift(newR);
                this.newReview.rv_rate = 5;
                this.newReview.rv_comment = '';
